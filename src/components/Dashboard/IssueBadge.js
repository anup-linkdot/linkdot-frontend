            ...
            <div className='issue-badge-row'>
                ...
                <div className='issue-badge-ops'>
                    <div className='upload-btn-div'>
                        <button className='badge-input badge-input-file upload-btn' onClick={() => clickInput()}><p>Upload CSV</p></button>
                        <input type='file' id="getFile" accept={".csv"} onChange={(e) => getCSVFile(e.target.files[0])} />
                        <img alt='' className='info-img' src={Info} onClick={() => setShowHover(!show_hover)} />
                        {show_hover === true &&
                            <div className='info-hover-div'>
                                <p>Please upload csv with header as 'Email ID' and all the emails in the below rows</p>
                            </div>
                        }
                        <p className='or-text'>or</p>
                    </div>

                    <textarea className='badge-description issue-email-textarea' placeholder='Enter User Email IDs, separated by comma' defaultValue={emails}
                        onChange={(e) => setEmails(e.target.value)}>
                    </textarea>
                    <p className='email-text'>Enter User Email ID separated by comma or upload CSV file for bulk Share, click file icon for Uploading CSV file</p>
                    {loader === false ?
                        <button className='share-badge-btn' onClick={() => shareBadge()}>Share Badge</button>
                        :
                        <button className='share-badge-btn loading-share-badge' disabled><img alt='' src={Loader} />Sharing Now</button>
                    }
                </div>
            </div>
        </div>
