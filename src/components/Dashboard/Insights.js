                    ...
                    {loading === false ?
                        <table className='insights-table'>
                            <tr className='table-keys'>
                                ...
                                <th className='key-text'>{active_tab === "issued" ? 'Issued To' : "Issued From"}</th>
                            ...
                            {active_tab === "issued" ?
                                <React.Fragment>
                                    {badges.map((badge, index) => {
                                        return (
                                            <tr className='table-keys  table-values' key={badge._id + + Math.random()}>
                                                <td className='key-value'>{index + 1}</td>
                                                <td className='key-value'>{badge.name}</td>
                                                <td className='key-value'>{badge.badge_type}</td>
                                                <td className='key-value'>{moment(badge.issued_date).format("MMYYYY")}</td>
                                ...
                                <React.Fragment>
                                    {badges.map((badge, index) => {
                                        return (
                                            <tr className='table-keys  table-values' key={badge.badge_id._id + Math.random()}>
                                                <td className='key-value'>{index + 1}</td>
                                                <td className='key-value'>{badge.badge_id.name}</td>
                                                <td className='key-value'>{badge.badge_id.badge_type}</td>
                                                <td className='key-value'>{moment(badge.badge_id.issued_date).format("MMYYYY")}</td>
